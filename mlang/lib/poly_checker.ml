...
let make_subst : var -> typ -> subst =
 fun x t ->
  let rec subs t' =
    ...
    | TPair (t1, t2) ->
        TPair (subs t1, subs t2)
    | TLoc t'' ->
        TLoc (subs t'')
    | TFun (t1, t2) ->
        TFun (subs t1, subs t2)
    | _ ->
        t'
  in
  subs
...
let subst_scheme : subst -> typ_scheme -> typ_scheme =
 fun subs tscm ->
  match tscm with SimpleTyp t -> SimpleTyp (subs t) | GenTyp (alphas, t) -> GenTyp (alphas, subs t)


let subst_env : subst -> typ_env -> typ_env =
 fun subs tenv -> List.map (fun (x, tscm) -> (x, subst_scheme subs tscm)) tenv
